...
export default function AddScheme() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await adminService.addScheme(form);
      if (res.success) {
        toast.success("Scheme added successfully!");
        navigate("/admin/schemes");
      } else {
        toast.error(res.message || "Failed to add scheme");
      }
    } catch {
  ...
  return (
    <div className="page-container py-8 max-w-3xl animate-fade-in">
      ...
      >
        <h1 className="section-title mb-1">Add New Scheme</h1>
        <p className="text-slate-500 text-sm mb-8">
          Fill in the details to create a new government scheme entry.
        </p>
          ...
          >
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                placeholder="Describe the scheme's objectives, benefits, and key features…"
                rows={4}
            ...
            <div className="sm:col-span-2 flex gap-4 mt-2">
              ...
              >
                {loading ? <Loader size="sm" /> : "✅ Add Scheme"}
              </motion.button>
            </div>
          </form>
        </div>
